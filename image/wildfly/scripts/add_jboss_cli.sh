...
BATCH_FILES=$(comm -23 --nocheck-order \
  <(ls /entrypoint-wildfly-cli ${WILDFLY_HOME}/internal_cli 2> /dev/null | sort | grep -v "/" | grep -E "$CLI_FILTER$" | grep -v .completed) \
  <(ls --sort=name ${MOS_READY_PATH} 2> /dev/null | grep .completed | sed 's/\.completed$//'))
...
if [ $(echo ${BATCH_FILES} | wc -w) -gt 0 ]; then
  ...
  bash ${WILDFLY_HOME}/bin/standalone.sh --admin-only &
  until `bash ${JBOSS_CLI} -c ':read-attribute(name=server-state)' 2> /dev/null | grep -q running`; do sleep 1; done;

  for BATCH_FILE in ${BATCH_FILES}; do
    if [ -f "/entrypoint-wildfly-cli/${BATCH_FILE}" ]; then
      echoInfo "execute jboss-batchfile \"${BATCH_FILE}\""
      bash ${JBOSS_CLI} -c --properties=env.properties --file=/entrypoint-wildfly-cli/${BATCH_FILE}
    elif [ -f "${WILDFLY_HOME}/internal_cli/${BATCH_FILE}" ]; then
      echoInfo "execute internal jboss-batchfile \"${BATCH_FILE}\""
      bash ${JBOSS_CLI} -c --properties=env.properties --file=${WILDFLY_HOME}/internal_cli/${BATCH_FILE}
    fi
    if [ $? -eq 0 ]; then
      touch ${MOS_READY_PATH}/${BATCH_FILE}.completed
    else
      echoErr "JBoss-Batchfile \"${BATCH_FILE}\" can not be execute"
      bash ${JBOSS_CLI} -c ':shutdown'
  ...
  bash ${WILDFLY_HOME}/bin/jboss-cli.sh -c ':shutdown'
fi
